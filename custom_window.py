...
class Taskbar:
    ...
    def is_auto_hide():
        ...
        taskbar_state = windll.shell32.SHAppBarMessage(
            ABM_GETSTATE, byref(appbar_data))
        return taskbar_state == ABS_AUTOHIDE
...
class CustomBase(QWidget):
    ...
    def set_effect(self, enable=True):
        ...
        if enable and self.use_mica:
            ...
            #print(win32api.GetAsyncKeyState(0x01)&0x8000)
...
class CloseButton(TitleBarButton):
    def __init__(self, parent, dark_mode):
        ...
        self.setStyleSheet(style)
        self._white_icon = QIcon(r"res\close_white.png")
        if self._dark_mode:
            self.setIcon(self._white_icon)
        else:
            self._black_icon = QIcon(r"res\m_close-black.png")
...
class CustomWindow(CustomBase):
    ...
    def setWindowTitle(self, title,Fonts="Segoe UI"):
        self.title_bar.title.setText(title)
        self.title_bar.title.setFont(Fonts)
    ...
    def resizeEvent(self, event):
        ...
        self.main_widget.setFixedWidth(self.width())
        self.main_widget.move(0,32)
        self.main_widget.setFixedHeight(self.height()-32)
